...
def get_args():
    ...
    parser.add_argument('--iters', type=int, default=1200000, help='Number of epochs')
    ...
    parser.add_argument('--rt_weight', type=float, default=0.2, help='weight of returb LR')
    ...
    parser.add_argument('--train_path', type=str, default='', help='path of training imgs')
    parser.add_argument('--train_info', type=str, default='', help='info of training imgs')
    parser.add_argument('--val_path', type=str, default='', help='path of validation imgs')
    parser.add_argument('--val_info', type=str, default='', help='info of testing imgs')   
    parser.add_argument('--load', '-f', type=str, default=False, help='Load model from a .pth file')
    parser.add_argument('--log_path', type=str, default='~', help='path to save logging files and images')
    parser.add_argument('--reblur_path', type=str, default='../LPD_learning/model_zoo/NAF_decoder.pth', help='path to vae reblur model')
    parser.add_argument('--run_name', type=str, default='MambaTM_dynamic', help='name of this running')
...
def validate(args, model, val_loader, criterion, criterion_lpips, TS, iter_count, im_save_freq, im_save_path, device, level):
        ...
        for s, data in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                loss = criterion(output, target) + 0.01 * loss_lpips
                loss_all = loss + args.rt_weight * (loss_returb + 0.01 * kld)
                eval_loss += loss.item()
...
def main():
    ...
    for epoch in range(1000000):
        for data in train_loader:
            ...
            loss_lpips = criterion_lpips(output.flatten(0,1)*2-1, target.flatten(0,1)*2-1).mean()
            loss = criterion_char(output, target) + 0.01 * loss_lpips
            loss_all = loss + args.rt_weight * (loss_returb + 0.01 * kld)
            ...
            if iter_count>0 and iter_count % args.val_period == 0:
                psnr_w, ssim_w, loss_w, loss_rt_w, loss_kld_w = validate(args, model, val_loader_weak, criterion_char, criterion_lpips, \
                                                                         TS, iter_count, 200, result_img_path, device, 'weak')
                logging.info('Validation W: Iters {:d}/{:d} - Loss {:8f} - RTLoss {:8f} - KLD {:8f} - PSNR: {:.2f} dB; SSIM: {:.4f}'.format(iter_count, total_iters, loss_w, loss_rt_w, loss_kld_w, psnr_w, ssim_w))
                
                psnr_m, ssim_m, loss_m,loss_rt_m, loss_kld_m = validate(args, model, val_loader_medium, criterion_char, criterion_lpips, \
                                                                        TS, iter_count, 200, result_img_path, device, 'medium')
                logging.info('Validation M: Iters {:d}/{:d} - Loss {:8f} - RTLoss {:8f} - KLD {:8f} - PSNR: {:.2f} dB; SSIM: {:.4f}'.format(iter_count, total_iters, loss_m, loss_rt_m, loss_kld_m, psnr_m, ssim_m))
                
                psnr_s, ssim_s, loss_s, loss_rt_s, loss_kld_s = validate(args, model, val_loader_strong, criterion_char, criterion_lpips, \
                                                                         TS, iter_count, 200, result_img_path, device, 'strong')
                logging.info('Validation S: Iters {:d}/{:d} - Loss {:8f} - RTLoss {:8f} - KLD {:8f} - PSNR: {:.2f} dB; SSIM: {:.4f}'.format(iter_count, total_iters, loss_s, loss_rt_s, loss_kld_s, psnr_s, ssim_s))

...
def get_args():
    ...
    parser.add_argument('--run_name', type=str, default='MambaTM_static', help='name of this running')
...
def main():
    ...
    val_loader = DataLoader(dataset=val_dataset, batch_size=args.batch_size*2, shuffle=False, num_workers=args.num_workers, \
    ...
    scheduler_cosine = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, args.iters - args.warmup_iters, eta_min=1e-6)
    scheduler = GradualWarmupScheduler(optimizer, multiplier=1, total_epoch=args.warmup_iters, after_scheduler=scheduler_cosine)
    ...
    for epoch in range(1000000):
        ...
        for data in train_loader:
            ...
            loss = criterion_char(output, target) + 0.01 * criterion_lpips(output.flatten(0,1)*2-1, target.flatten(0,1)*2-1).mean()
            loss_all = loss + args.rt_weight * (loss_returb + 0.01 * kld)
            loss_all.backward()

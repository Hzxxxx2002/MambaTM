...
def get_args():
    ...
    parser.add_argument('--resize', type=float, default=1.0, help='target size ratio')
    parser.add_argument('--patch_size', type=int, default=256, help='patch size')
    parser.add_argument('--output_folder_name', '-oname', type=str, default="default", help='output directory name')
    parser.add_argument('--input_folder_name', '-iname', type=str, default="default", help='input directory name, the input frames are stored as 00000001.png...')
...
with torch.no_grad():
    for v in os.listdir(input_dir)[::-1]:
        ...
        all_frames = read_images(input_path, "{:08d}.png", start_id=start_frame, length=args.n_frames, resize=resize) # 256:0.581
        ...
        for j in range(num_out):
            out = cv2.cvtColor(tensor2img(output[...,:h, :w], j), cv2.COLOR_RGB2BGR)
            out = cv2.resize(out, (int(w/resize), int(h/resize)))
